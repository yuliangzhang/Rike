...
///    全程在同目录 `flock` 内，顺序是：读目标算 hash → 校验 → 写临时文件并 fsync →
///    **再复核一次目标 hash** → rename。关键是**复核紧挨着 rename**：
///    耗时的写盘在复核之前完成，残余窗口只是「复核 → rename」的几十微秒，
///    而不是包含写盘 + fsync 的毫秒级窗口。此时目标内容本就是我们的产物。
